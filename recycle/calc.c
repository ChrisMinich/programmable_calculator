...
string entry;
string keys[] = { "STO", "RCL", "POP", "/", "*", "-", "+", "Y^X", "INT", "ABS", "%", "OFF", "STACK", "CLEAR", "KEYS", "LBL", "ISZ", "DSZ", "N[I]", "X<0", "X>0", "X>1", "X=Y", "X>Y", "X!=0" ,"GTO", "GSB", "HELP", "RTN", "R/S", "SST", "S", "BST", "B", "FST", "F", "FLOAT", "PAUSE", "WHERE", "BEGIN", "LABELS", "LOAD", "SHELL", "Q" };
...
int parse(string entry, int mode) // 0: interactive, 1: program, 2: trace
{
	...
	else if ( strcmp(entry, "PAUSE") == 0 ) printfloat(x);
    ...
    else // push(num) unless entry isn't numeric and not a member of keys[]
    {
        ...
        if (num == 0)
            ...
            {
            	if (find(entry)) // run program at label (GSB)
            	{
            		mv_step(1);
            		start(NO_TRACE);
